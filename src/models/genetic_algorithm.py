...
class GeneticAlgorithmTrainer:
    def __init__(self, population_size=20, mutation_rate=0.1, generations=50, selection_rate=0.5, hidden_size=15):
    ...
    def initialize_individual(self, model):
        return [param.data.clone() + torch.randn_like(param) * 0.1 for param in model.parameters()]
    ...
    def train(self, X_train, y_train):
        ...
        population = [self.initialize_individual(model) for _ in range(self.population_size)]
        best_fitness = 0.0
        losses = []
        
        for generation in range(self.generations):
            fitness_scores = [self.evaluate_fitness(model, ind, X_train, y_train) for ind in population]
            gen_best_fitness = max(fitness_scores)
            best_fitness = max(best_fitness, gen_best_fitness)
            
            print(f"Generation {generation+1} - Best Fitness: {gen_best_fitness:.4f}")
            losses.append(1 - gen_best_fitness)  # Store loss as (1 - fitness) for tracking
            
            parents = self.select_parents(population, fitness_scores)
            next_generation = [self.initialize_individual(model) for _ in range(self.population_size)]
